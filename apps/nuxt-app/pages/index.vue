...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 12, fontStyle: 'italic', color: '#888', textAlign: 'center', marginBottom: 30 },
...
<template>
  <div style="display: flex; flex-direction: column; height: 100vh; font-family: system-ui, sans-serif; background: #f8f9fa">
    ...
    <div v-if="tab === 'demo'" style="display: flex; flex-direction: column; flex: 1; padding: 16px; gap: 12px">
      <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap">
        <h2 style="white-space: nowrap; margin: 0">Nuxt Demo</h2>
        <input v-model="title" placeholder="PDF Title"
          style="flex: 1; min-width: 200px; padding: 8px 12px; border: 1px solid #ccc; border-radius: 6px; font-size: 14px" />
        <select v-model="mode"
        ...
        <PDFViewer width="100%" style="flex: 1; border-radius: 8px; overflow: hidden">
          <PDFDocument :title="title" author="PDFCraft">
            <PDFPage size="A4" :style="styles.page">
              ...
              <PDFText :style="styles.subtitle">Generated client-side with declarative components</PDFText>
              <PDFView :style="styles.section">
                <PDFText :style="styles.sectionTitle">Declarative API</PDFText>
                <PDFText :style="styles.text">This PDF is built using Vue template components. Change the title above and watch it re-render reactively.</PDFText>
              </PDFView>
            </PDFPage>
          </PDFDocument>
        </PDFViewer>
