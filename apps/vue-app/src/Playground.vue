...
// Return a PDFNode tree using the core API
return Document({ title: 'PDFCraft Playground', author: 'PDFCraft' }, [
  Page({ size: 'A4', style: styles.page }, [
    Text({ style: styles.header }, 'Hello PDFCraft!'),
    Text({ style: styles.subtitle }, 'Edit this code on the left and see the PDF update live'),

    View({ style: styles.row }, [
      View({ style: styles.card }, [
        Text({ style: styles.cardTitle }, 'Flexbox Layout'),
    ...
    View({ style: { flexDirection: 'row', gap: 6, marginBottom: 14 } }, [
      Text({ style: styles.badge }, 'Vue'),
      Text({ style: styles.badge }, 'React'),
      Text({ style: styles.badge }, 'Svelte'),
      Text({ style: styles.badge }, 'SSR'),
    ]),

    View({ style: styles.section }, [
      Text({ style: styles.sectionTitle }, 'Try it out!'),
      Text({ style: styles.text },
        'Change the text, add more Views, adjust styles — the PDF regenerates automatically. Use Document(), Page(), View(), Text(), Image(), Link() and StyleSheet.create().'
      ),
    ]),

    Text({ style: styles.footer }, 'PDFCraft Playground — pdfkit + yoga layout engine'),
  ]),
])
`
...
function compileAndRun(code: string): { tree: PDFNode | null; error: string | null } {
  try {
    ...
    const scope = { Document, Page, View, Text, Image, Link, StyleSheet, Font }
    ...
    return { tree: null, error: 'Code must return a Document() node.' }
  } catch (err: any) {
