...
export default function Playground() {
  ...
  const generatePDF = useCallback(async (source: string) => {
    ...
    setLoading(true)
    setError('')
    const { tree, error: compileError } = compileAndRun(source)
    if (compileError || !tree) {
      setError(compileError || 'No document returned.')
      setLoading(false)
      return
    }
    try {
...
const wrapStyle: CSSProperties = { display: 'flex', flexDirection: 'column', height: '100%', background: '#0f172a', color: '#e2e8f0' }
